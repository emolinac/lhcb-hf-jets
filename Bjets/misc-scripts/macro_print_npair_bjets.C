...
void macro_print_npair_bjets(const int bin = 0)
{
        ...
        for (int bin = 0 ; bin < 3 ; bin++) {
                ...
                h_heavy[bin]    = (TH1F*) f_heavy->Get(Form("hdata_npair_corr%i", bin + 4));
                h_heavy_mc[bin] = (TH1F*) f_heavy->Get(Form("hmc_npair%i"  , bin + 4));
                
                set_histogram_style(h_heavy[bin], corr_marker_color_jet_pt[bin], std_line_width-1, corr_marker_style_jet_pt[bin], std_marker_size+1);
                set_histogram_style(h_heavy_mc[bin], corr_marker_color_jet_pt[bin], std_line_width-1, std_marker_style_jet_pt[bin], std_marker_size+1);

                hs[bin]->Add(h_heavy[bin], "APE");
                hs[bin]->Add(h_heavy_mc[bin], "APE");
                l[bin]->AddEntry(h_heavy[bin],"b-jet data","p");
                l[bin]->AddEntry(h_heavy_mc[bin],"b-jet PYTHIA8","p");
        }
...
void draw_three_pads_eec(THStack* h_jetpt_0, THStack* h_jetpt_1, THStack* h_jetpt_2, TLegend* l0, TLegend* l1, TLegend* l2, TLatex* lhcb_print)
{
        ...
        h_jetpt_1->SetTitle(";#it{R_{L}};N_{pair}(#it{R_{L}})");
        h_jetpt_1->GetXaxis()->SetRangeUser(0.02,0.5);
        h_jetpt_1->SetMaximum(50);
        ...
        h_jetpt_2->SetTitle(";#it{R_{L}};N_{pair}(#it{R_{L}})");
        h_jetpt_2->GetXaxis()->SetRangeUser(0.02,0.5);
        h_jetpt_2->SetMaximum(50);
        ...
        c->Print("../plots/npair-b-jet-data-corr.pdf");
}