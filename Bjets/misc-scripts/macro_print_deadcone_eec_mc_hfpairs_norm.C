...
void macro_print_deadcone_eec_mc_hfpairs_norm(const int bin = 0)
{
        ...
        for (int bin = 0 ; bin < 3 ; bin++) {
                h_heavy[bin] = (TH1F*) f_heavy->Get(Form("hmc_eec_whf%i", bin + 4));
                h_light[bin] = (TH1F*) f_light->Get(Form("hcorr_eec_truth%i", bin));

                h_heavy[bin]->Scale(1./h_heavy[bin]->Integral(),"width");
                h_light[bin]->Scale(1./h_light[bin]->Integral(),"width");

                h_deadcone[bin] = new TH1F(Form("h_deadcone%i", bin), "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning);

                h_heavy_data[bin] = (TH1F*) f_heavy->Get(Form("hdata_eec_whf%i", bin + 4));
                h_light_data[bin] = (TH1F*) f_light_data->Get(Form("hcorr_eec%i", bin));
                h_deadcone_data[bin] = new TH1F(Form("h_deadcone_data%i", bin), "", nbin_rl_nominal_unfolding, unfolding_rl_nominal_binning);

                h_heavy_data[bin]->Scale(1./h_heavy_data[bin]->Integral(),"width");
                h_light_data[bin]->Scale(1./h_light_data[bin]->Integral(),"width");
                ...
                for (int i = 1 ; i < h_deadcone[bin]->GetNbinsX(); i++) {
                        ...
                        double light_eec_interpolation_data = h_light_data[bin]->Interpolate(h_heavy_data[bin]->GetBinCenter(i));

                        h_deadcone_data[bin]->SetBinContent(i, h_heavy_data[bin]->GetBinContent(i) / light_eec_interpolation_data);
                }
